...
function kalilagi(num, i, jumlah) {
    if (i === jumlah) {
      return 1;
    } else {
      return Number(num[i]) * kalilagi(num, i+1, jumlah);
    }
  }
  
  function kaliTerusRekursif(angka) {
    var angka_string = angka.toString();
    while (angka_string.length > 1) {
      angka_string = kalilagi(angka_string, 0, angka_string.length).toString();
    }
    return Number(angka_string);
  }
